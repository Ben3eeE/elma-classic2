...
std::optional<rec_header> recorder::read_header(const std::string& filename) {
    ...
    rec_header header{};
    if (fread(&header.level_id, 1, sizeof(header.level_id), h.get()) != sizeof(header.level_id)) {
        return std::nullopt;
    }
    if (fread(header.level_filename, 1, sizeof(header.level_filename), h.get()) !=
        sizeof(header.level_filename)) {
        return std::nullopt;
    }
