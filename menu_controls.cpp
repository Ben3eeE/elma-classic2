...
constexpr int FUNCTION_KEYS_END = FUNCTION_KEYS_START + 15;
...
static void load_function_controls(key_pointers keys) {
    ...
    load_control(keys, i++, "Function key 13", &State->key_function_13);
    load_control(keys, i++, "Function key 14", &State->key_function_14);
    load_control(keys, i++, "Function key 15", &State->key_function_15);
}
...
static void menu_customize_function_keys(key_pointers keys) {
    ...
    while (true) {
        ...
        if (choice >= 0 && choice < FUNCTION_KEYS_END) {
            prompt_control(FUNCTION_KEYS_END, keys, choice);
        }
    }
}
