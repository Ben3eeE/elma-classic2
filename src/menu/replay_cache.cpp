...
void replay_cache::sync(const std::vector<std::string>& current_filenames) {
    ...
    for (const auto& [filename, _] : filename_to_level_) {
        if (!on_disk.contains(filename)) {
            stale.push_back(filename);
        }
    }
    for (const auto& filename : stale) {
        int level_id = filename_to_level_[filename];
        auto& bucket = entries_[level_id];
        std::erase(bucket, filename);
        filename_to_level_.erase(filename);
    }

    // Add new files
    for (const auto& filename : current_filenames) {
        if (!filename_to_level_.contains(filename)) {
            auto header = recorder::read_header(filename);
            if (header) {
                apply_one(filename, header->level_id);
            }
        }
    }
}
