...
// This is called after the window has finished resizing.
void window_resize_event(int width, int height) {
    // Don't save the new size to EolSettings
    SCREEN_WIDTH = width;
...
void handle_events() {
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_KEYDOWN: {
            ...
            // On X11/Windows, SDL suppresses TEXTINPUT while Ctrl is held, so
            // resolve the layout-specific keycode here to support LCtrl search.
            // Wayland delivers TEXTINPUT anyway and is de-duplicated below.
